...
exports.estadisticasGrupos = async ({ fkCarrera, fkPeriodo }) => {
  try {
    ...
    for (const [grupoPk, califs] of Object.entries(calificacionesPorGrupo)) {
      ...
      // Obtener la cantidad de materias asociadas al grupo paratir de fk carrera y fk periodo
      const materiasUnicas = await ListCalif.findAll({
        attributes: ['fk_materias'],
        where: {
            fk_Periodos: fkPeriodo,
            fk_grupos: grupoPk
        },
        include: [{
          model: Grupos,
        ...
        raw: true,
        distinct: true // Asegura que se cuenten solo las materias únicas
      });
