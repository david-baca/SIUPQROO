...
const DirectorTD: React.FC<UserTableRowProps> = ({ user }) => {
  ...
  const [selectedCareer, setSelectedCareer] = useState<number | null>(
    user.directorCareerId
  );
  ...
  const fetchCareersAndDirector = async () => {
    try {
      ...
      const allCareers = [
        { pk: 0, nombre: 'Sin permiso', cant_grup: 0, p_ap: 0, p_rep: 0 },
      ...
      if (directorResponse.data != null) {
        ...
        const foundCareer = allCareers.find(
          (career) => career.pk === selectedCareerId
        );
        if (foundCareer) {
  ...
  const handleCareerChange = async (
    ...
    try {
      ...
      if (careerId !== 0) {
        await axios.post('/user/directors/create', {
          fk_Usuario: user.pk,
          fk_Carrera: careerId
        });
      }
