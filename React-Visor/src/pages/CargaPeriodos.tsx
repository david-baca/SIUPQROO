...
const VistaCargaPeriodos = () => {
  ...
  return response ? (
    <Estatus response={response} />
  ) : (
    <CargaPeriodos setResponse={setResponse} />
  );
};
...
const CargaPeriodos: React.FC<CargaPeriodosProps> = ({ setResponse }) => {
  ...
  const handleCuatrimestreChange = (
    ...
    const cuatrimestre = cuatrimestres.find(
      (c) => c.cuatrimestre === e.target.value
    );
    const newSelections = [...cuatrimestreSeleccionado];
    newSelections[index].cuatri = cuatrimestre
  ...
  const esValido = () => {
    return cuatrimestreSeleccionado.some(
      (selection) => selection.cuatri !== 'Cuatrimestre' && selection.anio !== 0
    );
  };

  const obtenerFechasValidas = async () => {
    const validos = cuatrimestreSeleccionado.filter(
      (selection) => selection.cuatri !== 'Cuatrimestre' && selection.anio !== 0
    );
      ...
      contenido={
        <>
          ...
          <div className="flex items-center justify-between py-2 w-full px-5">
            ...
            <div>
              <button
                onClick={handleNavigateCargaDatos}
      ...
      contenido={
        <>
                    ...
                    <span className="w-100 flex justify-center">
                      {item.estado === 'activo' ? (
                        ...
                        <svg
                          ...
                          height="50"
                          fill="#FFFFFF"
                          className="bg-[#EDE63B] rounded-full bi bi-exclamation-circle-fill"
                          viewBox="0 0 16 16">
          ...
          <div className="flex items-center justify-between py-2 w-full px-5">
            ...
            <div>
              <button
                onClick={handleNavigateCargaPeriodo}
                className="text-gray-600 hover:text-gray-800 mx-2">
                Anterior
              </button>
              <button className="text-gray-600 mx-2 cursor-default">
                Siguiente
              </button>
            </div>
          </div>
