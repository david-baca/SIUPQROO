...
const UserPermissionsView: React.FC = () => {
  ...
  return (
    <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-center border-collapse">
                  <thead>
                    <tr className="bg-gray-200">
                      <th className="border border-[#b4b5b8] px-4 py-2">
                        Usuario
                      </th>
                      <th className="border border-[#b4b5b8] px-4 py-2">
                        Director de Carrera
                  ...
                  <tbody>
                    {users.map((user) => (
                      <tr key={user.pk}>
                        <td className="border border-[#b4b5b8] px-4 py-2">
                          {user.correo}
                        </td>
                        <td className="border border-[#b4b5b8] px-4 py-2">
                        ...
                        <td className="border border-[#b4b5b8] px-4 py-2">
                          <div className="flex justify-center space-x-2">
                            <button
                              onClick={() => handleClickOpenEdit(user.pk)}
                              className="bg-yellow-500 text-white px-4 py-2 rounded-md shadow-sm hover:bg-yellow-600 flex items-center space-x-1">
                              <svg
                                className="w-5 h-5"
                            ...
                            <button
                              onClick={() => handleClickOpenDelete(user.pk)}
                              className="bg-red-500 text-white px-4 py-2 rounded-md shadow-sm hover:bg-red-600 flex items-center space-x-1">
                              <svg
                                className="w-5 h-5"
                                xmlns="http://www.w3.org/2000/svg"
                                height="24px"
                ...
                <div className="flex flex-col items-end">
                  <button
                    ...
                    className="bg-[#ff8702] text-white p-1 mt-9 px-5 rounded-3xl">
      ...
      {openCreate && <FloadCreating handleCloseCreate={handleCloseCreate} />}

      {openEdit.isOpen && (
        <FloadEditing
          user={users.find((user) => user.pk === openEdit.userId)}
