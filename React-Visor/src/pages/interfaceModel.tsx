...
interface Props {
  ...
  homeText?: string; // Nueva prop opcional para el texto "Inicio"
}

const interfaceModel: React.FC<Props> = ({ userType, titleSection, titleAction, subtitleAction, contenido, homeText }) => {
    ...
    <div className="h-[100vh] w-[100vw] flex flex-col items-center">
      <header className="sticky top-0 bg-white w-[100vw] px-10 py-1 flex justify-between border-gray-300 border-b-[3px]">
        ...
        <div className="flex items-center relative">
          {homeText && <span className="mr-4 cursor-pointer" onClick={handleNavigateHome}>{homeText}</span>}
          <div onClick={() => setMostrarCerrarSesion(!mostrarCerrarSesion)} className="flex items-center cursor-pointer">
            <span className="pr-2">
          ...
          {mostrarCerrarSesion && (
            <div className='w-[120%] absolute bottom-[-140%]'>
              ...
              <div className="bg-orange-500 text-white p-2 font-bold flex-col">
                ...
                <h1 onClick={exit} className='inline-block p-0 cursor-pointer'>
                  Cerrar sesión
                </h1>
              </div>
            </div>
